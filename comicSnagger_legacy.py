...
def collect_image_data(chapters):
    ...
    for chapter in chapters:
        ...
        print(f"  {title} has {page_count} pages")

    print(f"Total images to be processed: {combined_total}")
...
def scrape_links(url):
    ...
    # to avoid multiple get requests, we're only going to check and see if the
    # image exists
    ...
    while True:
        ...
        image_url = f"{page_url}/{img_num}.{ext}"
    ...
    for comic in comicbooks.keys():
        ...
        if not os.path.isfile(comic_dir + ".cbz"):
            for img in comicbooks[comic]:
                ...
                cmd = f'wget --no-verbose --show-progress -c {img} -O ' \
                      f'"{filename}"'
                os.system(cmd)

            compress_comic(comic_dir)
        else:
            print(f'"{comic_dir.split("/")[-1])}" already exists, skipping')
...
def compress_comic(comic_dir):
    ...
    if os.path.isdir(comic_dir):
        print(f"Creating comic book file: {comic_dir.split('/')[-1])}.cbz")
...
def save(directory, title, dictionary):
    ...
    with open(f"{comic_log}.json", "w") as f:
        json.dump(dictionary, f)


def main():
    """
    Get the comic's main page and follow the links to get all of the chapters
    that are available
    """
